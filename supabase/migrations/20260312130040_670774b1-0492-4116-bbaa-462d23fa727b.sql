...
-- Add gif_url column to point_transactions
ALTER TABLE public.point_transactions ADD COLUMN gif_url TEXT;

-- Update transfer_points_between_users to accept gif_url
CREATE OR REPLACE FUNCTION public.transfer_points_between_users(
  ...
  transfer_gif_url text DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
