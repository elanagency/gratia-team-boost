...
DELETE FROM member_monthly_spending 
WHERE user_id NOT IN (SELECT id FROM profiles);

DELETE FROM monthly_points_allocations 
WHERE user_id NOT IN (SELECT id FROM profiles);

DELETE FROM carts 
WHERE user_id NOT IN (SELECT id FROM profiles);

DELETE FROM backup_users 
WHERE original_user_id NOT IN (SELECT id FROM profiles);
