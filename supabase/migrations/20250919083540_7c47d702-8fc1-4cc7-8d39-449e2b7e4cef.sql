-- Update transfer_points_between_users function to handle point updates directly
...
BEGIN
  ...
  -- Start transaction block for atomic updates
  BEGIN
    -- Deduct points from sender's monthly_points
    UPDATE public.profiles 
    SET monthly_points = monthly_points - points_amount
    WHERE id = sender_user_id AND company_id = transfer_company_id;
    
    -- Add points to recipient's recognition points
    UPDATE public.profiles 
  ...
  EXCEPTION WHEN OTHERS THEN
    -- Handle any errors during the transaction - PostgreSQL will automatically rollback
    RETURN jsonb_build_object(
      'success', false, 
