...
-- Step 2: Make profiles.id nullable to allow orphaned profiles
ALTER TABLE public.profiles 
...
-- Step 4: Rename columns to be more explicit about referencing profiles
ALTER TABLE public.company_members 
...
BEGIN
  ...
  IF NEW.raw_user_meta_data->>'companyName' IS NOT NULL THEN
    ...
    -- Add user as admin of the new company with 0 initial points (gets monthly allocation)
...
-- Step 7: Update database functions to use profile_id instead of user_id
