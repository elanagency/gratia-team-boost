...
interface EmailServiceRequest {
  type: 'invitation' | 'welcome' | 'password-reset' | 'custom';
  to: string;
  toName: string;
  data: {
    companyName?: string;
...
const generateInvitationTemplate = (data: any) => {
  const { toName, companyName, isNewUser, email, password, origin } = data;
  ...
  const htmlContent = `
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          <h1 class="welcome-message">Hello ${toName}!</h1>
...
const generateEmailTemplate = (request: EmailServiceRequest) => {
  const { type, toName, data } = request;
  
  switch (type) {
    case 'invitation':
      return generateInvitationTemplate({ toName, ...data });
    
    case 'welcome':
      return {
        subject: `Welcome to ${data.companyName}!`,
        htmlContent: `<h1>Welcome ${toName}!</h1><p>Welcome to ${data.companyName}.</p>`
      };
    
    case 'password-reset':
      return {
        subject: 'Password Reset Request',
        htmlContent: `<h1>Password Reset</h1><p>Hello ${toName}, click the link to reset your password.</p>`
      };
    
    case 'custom':
      return {
        subject: data.customSubject || 'Custom Email',
        htmlContent: data.customTemplate || `<p>Hello ${toName}</p>`
      };
...
const sendEmail = async (to: string, toName: string, subject: string, htmlContent: string, companyName?: string) => {
  ...
  const brevoResponse = await fetch("https://api.brevo.com/v3/smtp/email", {
    ...
    body: JSON.stringify({
      ...
      tags: ["grattia-platform", companyName?.toLowerCase().replace(/\s+/g, '-') || "general"]
    }),
  });
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log("[EMAIL-SERVICE] Processing email request:", { type: request.type, to: request.to });
    ...
    const { subject, htmlContent } = generateEmailTemplate(request);
